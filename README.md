...
![101552](https://github.com/user-attachments/assets/8c042cc8-12f7-4da2-9e99-eac85626948e)

#### [:root 가상 클래스 사용방법 예시]  
##### 웹 문서 구조에서 특정 위치에 있는 요소를 선택하는 가상 클래스이다.<br/>
:root 선택자는 웹 문서 구조에서 가장 상위 요소를 선택할 때 사용한다.<br/>
...
##### CSS :root 변수 선언<br/>
- 웹 문서에서 공통으로 사용될 속성을 미리 변수에 선언한다.<br/>
--(하이픈 2개) 다음 속성 이름을 정해주고 : (콜론) 뒤에 속성 값을 적어준다.<br/>
...
b>파일명Ver숫자.jsx</b>로 작성을 해서 사용을 해주시면 됩니다. Ver = Version을 뜻하며 <br/>
작업 시 기존 파일은 백업하고 다시 사용하고 싶다면 말씀드린대로 네이밍을 해서 사용하시면 될 거 같습니다. <br/>
최종적으로 예시 정리해드리면 <b>파일명Ver숫자.jsx</b> 이런 형태입니다. <br/>
공지사항 리스트일 경우 <b>noticeListVer01.jsx</b> 이렇게 해주시고  <br/>
import 해주시면 오류 없이 불러올 수 있습니다.

## React 폴더 생성 및 역할<br/>
<img width="238" alt="image" src="https://github.com/user-attachments/assets/ea7304d2-f31a-4e17-97a7-f74c23074d0e"><br/>
- 저희는 작업 시 components, hooks, pages, styles, services 폴더를 만들어서 작업을 할 예정입니다.<br/>
  간단히 설명을 드리면<br/><br/>
  <b>components</b> : 재사용 가능한 컴포넌트들이 위치하는 폴더입니다.<br/>
  <b>pages</b> : 라우팅을 적용할 때 페이지 컴포넌트를 이 폴더에 넣습니다. 즉 서브 페이지 컴포넌트들을 넣습니다.<br/>
  <b>styles</b> : css를 합쳐놓은 폴더입니다.<br/>
  <b>services (= api)</b> : 보통 api관련 로직의 모듈 파일이 위치하는데 프론트나 백에 사용한 라이브러리 js source 등을 넣습니다.<br/><br/>
  <b>images</b> : 이미지 폴더는 public폴더 안에 images 폴더를 만들어서 사용하려고 합니다.<br/> 

  우선은 이 폴더만 생성해서 진행을 하려고 하고 추후에 작업하면서 필요한 폴더는 추가를 하면서 진행하겠습니다.<br/>
  하단에 [src 내부 폴더구조] 내용은 react 작업 시 기준이 되는 내용이니 확인해주세요.<br/><br/>
  
#### src 내부 폴더구조<br/>
![image](https://github.com/user-attachments/assets/7e11a035-1790-4f4a-88db-03415ffb7b52)<br/>

- components<br/>
  재사용 가능한 컴포넌트들이 위치하는 폴더입니다.<br/>
  컴포넌트는 매우 많아질 수 있기 때문에 이 폴더 내부에서 하위폴더로 추가로 분류하는 경우가 많습니다.<br/>
...
- constants<br/>
  공통적으로 사용되는 상수들을 정의한 파일들이 위치하는 폴더입니다.<br/>

- config<br/>
  config 파일이 많지 않은 경우 보통 최상위에 위치시켜놓지만 여러개의 config 파일이 있을 경우 폴더로 분리하기도 합니다.<br/>
...
- contexts<br/>
  contextAPI를 사용할 때 관련 파일들이 위치하는곳으로 상태관리를 위해 contextAPI 대신 redux를 사용 할 경우 폴더 이름을 store로 사용하기도 합니다.<br/>

위 내용을 따라서 폴더 구조를 분리 할 경우 이렇게 용도에 맞게 폴더별로 적절하게 분리된 모습을 볼 수 있습니다.<br/>
이 내용은 우선 참고만 해주세요<br/>
